...
func TestBuildCLIExplorationHintForCLIUsageGoal(t *testing.T) {
	hint := buildCLIExplorationHint("你学习一下这个 lark-cli 的使用可以不", []string{"sandbox_exec", "web_search"})
	if !strings.Contains(hint, "lark-cli") {
		t.Fatalf("expected command to be mentioned, got %q", hint)
	}
	if !strings.Contains(hint, "sandbox_exec") {
		t.Fatalf("expected sandbox_exec guidance, got %q", hint)
	}
