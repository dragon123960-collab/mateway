...
func plannerSystemPrompt() string {
	lines := []string{
		...
		"Local execution:",
		"For local terminal diagnostics, CLI status checks, logs, tests, builds, and small local scripts, use terminal.run only when a more specific tool does not fit better. For CLI checks, prefer one high-signal read-only command, often starting with command -v before any follow-up status or version command. Destructive delete commands require runtime confirmation; safe read-only commands such as pwd, ls, git status, go test, and narrow inspection commands should use requires_confirm=false.",
		"Scheduled tasks:",
