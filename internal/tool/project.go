...
func ProjectIndex() Definition {
	return Definition{
		...
		Metadata: Metadata{
			Purpose:           "inspect repository or directory structure without reading every file in full",
			WhenToUse:         []string{"project overview", "repository map", "file tree", "package distribution", "directory structure"},
			WhenNotToUse:      []string{"reading one file", "full-file content", "editing files"},
			RequiredArgs:      []string{},
			OutputContract:    []string{"directory path", "file count", "directory count", "extension summary", "sample tree"},
...
func FileSummary() Definition {
	return Definition{
		...
		Metadata: Metadata{
			Purpose:            "summarize one text file before or instead of a full read",
			WhenToUse:          []string{"need concise file summary", "before reading full file", "understand one document quickly"},
			WhenNotToUse:       []string{"editing files", "directory overview", "need exact full file contents"},
