...
func TestPendingProposalNudgeOncePerDay(t *testing.T) {
	...
	text, err := PendingProposalNudge(home, "cli:test", now)
	if err != nil {
		t.Fatal(err)
	}
	if !strings.Contains(text, "1 条长期记忆候选") {
		t.Fatalf("unexpected nudge: %q", text)
	}
	text, err = PendingProposalNudge(home, "cli:test", now.Add(time.Hour))
	if err != nil {
		t.Fatal(err)
	}
	if text != "" {
	...
	text, err = PendingProposalNudge(home, "cli:test", now.Add(24*time.Hour))
	if err != nil {
		t.Fatal(err)
	}
	if text == "" {
